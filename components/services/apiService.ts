...
export const submitUserPreferencesAndGenerate = async (
  ...
  if (initialPrefsResult.error) {
      ...
      return { error: accumulatedError.trim(), id: '', user_id: preferences.user_id, name: '', goal: '', duration_weeks: 0 };
  }
  ...
  return { 
    ...(programDetails.error ? {id: newProgramId, user_id: preferences.user_id, name: "Détails indisponibles", goal: preferences.goal || "N/A", duration_weeks: 0 } : programDetails),
...
export const updateUserPreferencesAndRegenerateProgram = async (
  ...
  try {
    ...
    if (prefsDetails.error) {
      return { error: `Erreur lors de la récupération des préférences actuelles: ${prefsDetails.error}`, id: '', user_id: userId, name: '', goal: '', duration_weeks: 0 };
    }
    currentFullPrefs = prefsDetails;
  } catch (e: any) {
    return { error: `Exception lors de la récupération des préférences: ${e.message}`, id: '', user_id: userId, name: '', goal: '', duration_weeks: 0 };
  }
  ...
  if (updatedPrefsResult.error) {
    ...
    return { error: accumulatedError.trim(), id: '', user_id: userId, name: preferencesToUpdate.name || currentFullPrefs.name || '', goal: preferencesToUpdate.goal || currentFullPrefs.goal || '', duration_weeks: 0 };
  }
  ...
  return { 
    ...(programDetails.error ? {id: newProgramId, user_id: userId, name: "Détails indisponibles", goal: finalPreferencesPayload.goal || "N/A", duration_weeks: 0 } : programDetails),
    sessions: sessionsArray, 
    message: genResponse.message,
    error: accumulatedError ? accumulatedError.trim() : undefined 
  };
